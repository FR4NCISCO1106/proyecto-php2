...
<body>
  ...
  <table>
    ...
    <?php
      if(@$_POST['buscar']) {
        $sql="select id,titulo,autor,fecha,categoria,pais from tema where (titulo like '%$_POST[buscar]%' or contenido like '%$_POST[buscar]%') order by id desc";
      } else if(@$_POST['autor']) {
        $sql="select id,titulo,autor,fecha,categoria,pais from tema where autor='$_POST[autor]' order by id desc";
      } else {
        $sql="select id,titulo,autor,fecha,categoria,pais from tema order by id desc";
      }
